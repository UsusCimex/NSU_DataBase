...
SELECT
    DATE_TRUNC('quarter', tt.arrival_time) AS quarter,
    EXTRACT(YEAR FROM tt.arrival_time) AS year,
    SUM(tt.tickets) OVER (PARTITION BY EXTRACT(YEAR FROM tt.arrival_time), DATE_TRUNC('quarter', tt.arrival_time) ORDER BY tt.arrival_time) AS cumulative_passengers
FROM
    timetable tt
GROUP BY
    quarter,
...
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT * FROM timetable LIMIT 100 LOOP
...
CREATE OR REPLACE FUNCTION fix_delays(target_date DATE)
RETURNS VOID AS $$
DECLARE
    train_record RECORD;
BEGIN
    FOR train_record IN SELECT t.train_id, w.value AS delay_minutes, tm.arrival_time, tm.departure_time
                        FROM waitings w
                        JOIN timetable tm ON tm.train_id = w.train_id
                        WHERE w.date = target_date AND w.value > 0
    LOOP
        UPDATE timetable
        ...
        WHERE train_id = train_record.train_id AND arrival_time = train_record.arrival_time;

        UPDATE waitings
        SET value = 0
        WHERE train_id = train_record.train_id AND date = target_date;
    END LOOP;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error: %', SQLERRM;
END;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM tmashrut tm
        JOIN trains t ON tm.marshrut_id = t.marshrut_id
        WHERE t.train_id = NEW.train_id AND tm.station_id = NEW.station_id
    ) THEN
...
CREATE TRIGGER check_train_station_route_before_insert
BEFORE INSERT OR UPDATE ON timetable
FOR EACH ROW EXECUTE FUNCTION check_train_station_route();
...
BEGIN
    ...
    SELECT order_num INTO cur_station_order_num
    FROM tmashrut
    ...
    IF cur_station_order_num > 0 THEN
        ...
        SELECT tms.station_id, tms.order_num INTO prev_station_id, prev_station_order_num
        FROM tmashrut tms
        ...
        SELECT departure_time INTO prev_departure_time
        FROM timetable
        WHERE train_id = NEW.train_id AND station_id = prev_station_id
        ORDER BY departure_time DESC
        LIMIT 1;

        -- Пытаемся найти средний интервал между аналогичными станциями для других поездов
        SELECT AVG(t2.arrival_time - t1.departure_time) INTO interval_between_stations
        FROM timetable t1
        JOIN timetable t2 ON t1.train_id = t2.train_id AND t1.station_id = prev_station_id AND t2.station_id = NEW.station_id
...
CREATE TRIGGER correct_timetable_timing_before_insert
BEFORE INSERT ON timetable
FOR EACH ROW EXECUTE FUNCTION correct_timetable_timing();
...
BEGIN
    SELECT COALESCE(MAX(marshrut_id), 0) + 1 INTO next_marshrut_id
    FROM (
        SELECT marshrut_id FROM marshrut
        UNION
        SELECT marshrut_id FROM tmashrut
    ) AS combined;
...
CREATE TRIGGER auto_assign_marshrut_id_before_insert
BEFORE INSERT ON marshrut
FOR EACH ROW EXECUTE FUNCTION auto_assign_marshrut_id();

-- Триггер для логирования удаления поездов
--создание таблицы
CREATE TABLE train_deletions_audit (
    train_id INT,
    deleted_tickets INT,
...
CREATE TRIGGER log_train_deletion_before_delete
BEFORE DELETE ON trains
FOR EACH ROW EXECUTE FUNCTION log_train_deletion();